...
export default function ExpensesPage() {
  ...
  useEffect(() => {
    obterDespesas().then(setExpenses);
  }, []);
  ...
  const onSubmit = async (data: z.infer<typeof expenseSchema>) => {
    if (editingExpense) {
      await atualizarDespesa(editingExpense.id, data);
    } else {
      await adicionarDespesa(data);
    }
    const updatedExpenses = await obterDespesas();
  ...
  const handleDelete = async (id: string) => {
    await excluirDespesa(Number(id));
    const updatedExpenses = await obterDespesas();
    setExpenses(updatedExpenses);
  };
              ...
              <TableCell>
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuItem
                      onSelect={() => handleDelete(String(expense.id))}
        ...
        <Dialog open={isDialogOpen} onOpenChange={handleOpenChange}>
          ...
          <DialogContent className="sm:max-w-[425px]">
            <form onSubmit={form.handleSubmit(onSubmit)}>
              <DialogHeader>
                <DialogTitle>
                  {editingExpense ? 'Editar' : 'Adicionar'} Despesa
                </DialogTitle>
                <DialogDescription>
      ...
      <Card>
        ...
        <CardContent>
          <Tabs defaultValue="fixed">
            ...
            <TabsContent value="fixed">{renderTable('fixed')}</TabsContent>
