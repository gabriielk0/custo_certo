...
const recipeIngredientSchema = z.object({
  ingrediente_id: z.coerce.number().min(1, 'Selecione um ingrediente'),
...
export default function RecipesPage() {
  ...
  const form = useForm<z.infer<typeof recipeSchema>>({
    ...
    defaultValues: {
      ingredientes: [{ ingrediente_id: 0, quantidade: 0 }],
    },
  });
    ...
    <div className="flex flex-col gap-8">
        ...
        <Dialog open={isDialogOpen} onOpenChange={handleOpenChange}>
          ...
          <DialogContent className="sm:max-w-4xl">
            <form onSubmit={form.handleSubmit(onSubmit)}>
              <DialogHeader>
                <DialogTitle>
                  {editingRecipe ? 'Editar' : 'Criar'} Receita
                </DialogTitle>
                <DialogDescription>
                  Preencha os detalhes da receita. O custo será calculado
                  automaticamente.
                </DialogDescription>
              </DialogHeader>
              <ScrollArea className="h-[60vh] p-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    />
                    {form.formState.errors.rendimento && (
                      <p className="text-red-500 text-xs mt-1">
                        {form.formState.errors.rendimento.message}
                      </p>
                    )}
                  </div>
                  <div>
                    ...
                    />
                    {form.formState.errors.peso_bruto && (
                      <p className="text-red-500 text-xs mt-1">
                        {form.formState.errors.peso_bruto.message}
                      </p>
                    )}
                  </div>
                  <div>
                ...
                <div className="mt-6">
                  ...
                  {fields.map((item, index) => (
                    <div key={item.id} className="flex gap-2 items-end mb-2">
                      ...
                      <div className="w-32">
                          ...
                          Qtd (
                          {
                            allIngredients.find(
                              (i) =>
                                i.id ==
                                form.watch(
                                  `ingredientes.${index}.ingrediente_id`,
                                ),
                            )?.unit
                          }
                          )
                        ...
                        <Input
                          type="number"
                          {...form.register(`ingredientes.${index}.quantidade`)}
                        />
                      </div>
                      <Button
                        type="button"
                  ...
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    onClick={() => append({ ingrediente_id: 0, quantidade: 0 })}
                  >
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Listagem de todos as suas receitas e produtos (ingredientes
            preparados).
          </CardDescription>
        </CardHeader>
        <CardContent>
